...
void DirectX9Renderer::ImGuiRender(struct ImDrawData *data, Microsoft::WRL::ComPtr<IDirect3DVertexBuffer9> &vbo,
                                   int &vboSize, Microsoft::WRL::ComPtr<IDirect3DIndexBuffer9> &ibo, int &iboSize) {
    ...
    if (!vbo || vboSize < data->TotalVtxCount) {
        ...
        if (FAILED(mDevice->CreateVertexBuffer(vboSize * sizeof(D3DVERTEX), D3DUSAGE_WRITEONLY, D3DVERTEX::kFVF,
    ...
    if (!ibo || iboSize < data->TotalIdxCount) {
        ...
        if (FAILED(mDevice->CreateIndexBuffer(iboSize * sizeof(ImDrawIdx), D3DUSAGE_WRITEONLY,
                                              sizeof(ImDrawIdx) == 2 ? D3DFMT_INDEX16 : D3DFMT_INDEX32, D3DPOOL_MANAGED,
                                              &ibo, NULL))) {
            return;
        }
    }
    ...
    ImDrawIdx *idx_dst;
    {
        ...
        for (int n = 0; n < data->CmdListsCount; n++) {
            ...
            for (int i = 0; i < cmd_list->VtxBuffer.Size; i++) {
                ...
                vtx_dst->z = 0.0f;
                vtx_dst->colour = (vtx_src->col & 0xFF00FF00) | ((vtx_src->col & 0xFF0000) >> 16) |
...
void DirectX9Renderer::doDraw(struct ImDrawList *cmd_list, int &vtx_offset, int &idx_offset) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
        ...
        if (pcmd->ElemCount > 0) {
            mDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, vtx_offset, 0, (UINT) cmd_list->VtxBuffer.Size,
...
TextureHandle DirectX9Renderer::CreateTexture(AbstractRenderer::eTextureFormats internalFormat, u32 width, u32 height,
                                              AbstractRenderer::eTextureFormats inputFormat, const void *pixels,
                                              bool /*interpolation*/) {
    LPDIRECT3DTEXTURE9 pTexture = nullptr;
    if (FAILED(mDevice->CreateTexture(width, height, 1, 0, ToD3DFormat(internalFormat), D3DPOOL_MANAGED, &pTexture,
                                      nullptr))) {
    ...
    if (pixels) {
        ...
        for (u32 y = 0; y < height; ++y) {
            for (u32 x = 0; x < width; x++) {
                ...
                if (inputFormat == AbstractRenderer::eTextureFormats::eRGBA ||
                    inputFormat == AbstractRenderer::eTextureFormats::eRGB) {
                    r = iPixelData[srcIdx++];
                    g = iPixelData[srcIdx++];
                    b = iPixelData[srcIdx++];
                }
                ...
                if (inputFormat == AbstractRenderer::eTextureFormats::eRGBA ||
                    inputFormat == AbstractRenderer::eTextureFormats::eA) {
                    a = iPixelData[srcIdx++];
                }
